...
\begin{description}
    ...
    \item[任务四] 实现系统可用内存查看功能。
    \item[任务五] 实现进程间页的传递，实现进程间通信的数据零拷贝。
...
\begin{table}[H]
    ...
    \begin{tabularx}{\linewidth}{lX}
        ...
        % C-core & 实现共享内存机制，实现\texttt{mprotect}系统调用，实现\texttt{brk/sbrk}系统调用及\texttt{malloc()/free()}库函数 \\
        C-core & 任务一、二、三、四、五。
        \bottomrule
    \end{tabularx}
...
\newpage %侵入式排版
\section{进程间页的直接传递}
从这里开始的部分只需要C-core的同学完成。

\subsection{任务四：查看可用内存}
本任务中，同学们需要完成对查看系统可用内存的支持，这样在测试时，可以更方便让我们查看系统资源使用情况。需要实现一个接口，并在Shell里增加一个命令:

\begin{description}
    \item[\texttt{size\_t sys\_get\_free\_memory()}] 返回系统可用内存，以字节为单位。
    \item[\texttt{free}]命令：输出系统可用内存，格式由同学自己定义，可以参考Linux中\texttt{free}命令，建议增加一个\texttt{-h}选项，方便阅读输出。
\end{description}

\subsection{任务五：内存页管道}
操作系统中 IPC 的性能十分关键，为了提升进程间通信的速度，我们可以直接将信息内容所在的页重映射到接收者的页表上，这样就实现了内容的零拷贝，可以提升 IPC 的性能。Linux中已有\texttt{splice}和\texttt{vmsplice}等相关函数，这次实验我们将实现类似的功能。
...
start code 内的测试程序代码在 \texttt{test/test\_project4}中。\texttt{pipe.c}会进行功能测试，\texttt{pipe\_self\_test}函数会在同一进程内创建管道并进行基本的读写测试；随后会创建两个子进程，分别调用\texttt{pipe\_sender}和\texttt{pipe\_receiver}，在不同进程之间进行管道读写测试。\texttt{ipc.c}会进行性能测试，分别通过mailbox和pipe传递大规模的数据并记录时间，验证管道是否实现数据的零拷贝传递。

同学们在实现时应考虑到换页机制的存在，管道机制应该处理好已被换出的页。在ipc.c中，测试数据为512MB，在初始化数据时，部分数据页面会被换到磁盘中，通过mailbox传输这些数据时，会从磁盘中读取页面，并导致其他数据页面被换入磁盘中，而通过pipe传输这些数据时，则不会涉及对磁盘数据的读取，这样大大加快数据传输的速度。

\begin{note}
同学们还应做好资源的回收与释放，在ipc测试前后，应该分别输出系统可用内存，测试前后可用内存数量应该大致保持不变。
\end{note}

\newpage %侵入式排版

对这些系统调用的行为说明如表\ref{tab:pipe}所示。
...
\begin{table}[H]
    ...
    \begin{tabularx}{\linewidth}{lX}
        ...
        参数/返回值  &  说明   \\
        \midrule
        \multicolumn{2}{l}{\texttt{int sys\_pipe\_open(const char *name)}} \\
        \midrule
        \texttt{name} & 要创建/打开的管道名称标识符 \\
        返回值 & 成功时返回管道描述符（非负整数），失败时返回\texttt{-1} \\
        \midrule
        \multicolumn{2}{l}{\texttt{long sys\_pipe\_give\_pages(int pipe\_idx, void *src, size\_t length)}} \\
        \midrule
        \texttt{pipe\_idx} & 目标管道的描述符（由\texttt{sys\_pipe\_open}返回） \\
        \texttt{src} & 要写入管道的源数据页起始地址（对齐到4KB） \\
        \texttt{length} & 要写入的数据长度（字节数，对齐到4KB） \\
        返回值 & 成功时返回实际写入的字节数，失败时返回\texttt{-1} \\
        \midrule
        \multicolumn{2}{l}{\texttt{long sys\_pipe\_take\_pages(int pipe\_idx, void *dst, size\_t length)}} \\
        \midrule
        \texttt{pipe\_idx} & 源管道的描述符（由\texttt{sys\_pipe\_open}返回） \\
        \texttt{dst} & 接收管道数据的目标内存地址（对齐到4KB） \\
        \texttt{length} & 请求读取的数据长度（字节数，对齐到4KB） \\
        返回值 & 成功时返回实际读取的字节数，失败时返回\texttt{-1} \\
        \bottomrule
    \end{tabularx}
    \caption{pipe 系统调用参数、返回值详细说明}
    \label{tab:pipe}
\end{table}

运行ipc测试效果如下，同学们在验收时，在执行测试前后，应使用\texttt{free -h}命令输出可用内存，确保资源被正确回收。
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{p4-pipe-demo}
    \caption{ipc测试效果}
    \label{fig:p4-Score-mem}
\end{figure}
